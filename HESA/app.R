...
df.propn.inc.staff$`HE Provider` = "All"


df %>%
    filter(income_outgoing == "inc") %>%
    group_by(`HE Provider`, year_ending) %>% 
    summarise(total_income_by_HE_Provider_and_year_UK = sum(value, na.rm = TRUE)) %>%
    left_join(df, by = c("HE Provider", "year_ending")) %>%
    filter(Category == "Staff costs") %>%
    group_by(`HE Provider`, year_ending, total_income_by_HE_Provider_and_year_UK) %>%
    summarise(total_staff_costs_by_year = sum(value, na.rm = TRUE)) %>% 
    mutate(perc = (total_staff_costs_by_year/
                       total_income_by_HE_Provider_and_year_UK) *100) -> df.propn.inc.staff.HE.provider




df.propn.inc.staff.HE.provider %>% 
    rbind(df.propn.inc.staff) ->
    df.plot
...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            selectizeInput(
                inputId = 'search',
                label = HTML('Search for unis<br/> Type name, and select via Enter<br/> Compare multiple unis'),
